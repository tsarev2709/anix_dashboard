(() => {
  ...
  function render(payload) {
    ...
    q('#cycleSummary').innerHTML = [cycleCard('Первое письмо → предоплата', cycles.first_touch_to_prepayment, 'полный цикл продажи'), cycleCard('Диалог → предоплата', cycles.dialog_to_prepayment, 'цикл после ответа клиента')].join('');
    const bottleneck = payload.bottleneck;
    q('#bottleneckRecommendation').innerHTML = bottleneck ? `<div><p class="eyebrow">Рекомендация</p><h3>Ускорить этап «${esc(bottleneck.stage_name)}»</h3><p>${esc(bottleneck.text)}</p></div><div class="bottleneck-metric"><strong>${days(bottleneck.average_days)}</strong><span>среднее время</span></div>` : '<strong>Пока недостаточно истории для определения узкого места.</strong>';
    ...
    q('#funnelConversions').innerHTML = conversions.length ? conversions.map((item, index) => `<div class="conversion-row"><div><strong>${esc(item.label)}</strong><small>${number(item.count)} вошли в этап за месяц</small></div><div class="conversion-bar"><i style="width:${item.from_previous === null ? 100 : Math.max(3, Math.min(100, item.from_previous * 100))}%"></i></div><div><strong>${index === 0 ? 'база' : pct(item.from_previous)}</strong><small>${index === 0 ? 'старт воронки' : `из «${esc(item.previous_label)}»`}</small></div></div>`).join('') : '<div class="empty-state">Нет переходов текущего месяца.</div>';
    ...
    q('#cashForecast').innerHTML = `<div class="cash-forecast-summary"><article><small>Взвешенный прогноз</small><strong>${money(forecast.weighted_amount)}</strong><span>${coverage < .5 ? 'низкая полнота бюджетов' : 'по вероятностям этапов'}</span></article><article><small>Сумма поздней воронки</small><strong>${money(forecast.raw_amount)}</strong><span>${pct(coverage)} карточек с бюджетом</span></article></div><div class="cash-stage-list">${(forecast.by_stage || []).map(item => `<div><span>${esc(item.label)} · ${item.count} сделок</span><strong>${money(item.weighted_amount)}</strong><small>${Math.round(item.probability * 100)}% от ${money(item.amount)}</small></div>`).join('')}</div><p class="history-caveat">${esc(forecast.method || '')}</p>`;
    ...
    q('#stageAging').innerHTML = stages.length ? stages.map(stage => {
      ...
      return `<div class="stage-aging-row"><div><strong>${esc(stage.stage_name)}</strong><small>${esc(stage.pipeline_name || 'Воронка')} · ${stage.count} сделок${esc(estimated)}${esc(noTask)}</small></div><div class="aging-bar"><i style="width:${width}%"></i></div><div class="aging-metrics"><strong>${days(stage.average_days)}</strong><small>медиана ${days(stage.median_days)} · максимум ${days(stage.max_days)}</small></div><div class="aging-oldest"><small>${oldest ? `Самые старые: ${esc(oldest)}` : '—'}</small></div></div>`;
  ...
  async function load() {
    ...
    q('#cycleUpdated').textContent = 'загрузка…';
    try {
      const requestHeaders = config.supabaseAnonKey ? { apikey: config.supabaseAnonKey, Authorization: `Bearer ${config.supabaseAnonKey}` } : {};
      const response = await fetch(`${config.supabaseUrl}/functions/v1/sales-cycle`, { headers: requestHeaders });
      ...
      q('#bottleneckRecommendation').innerHTML = `<strong>Не удалось загрузить аналитику: ${esc(error.message)}</strong>`;
      q('#cycleSummary').innerHTML = '<div class="empty-state">Циклы временно недоступны.</div>';
      q('#cashForecast').innerHTML = '<div class="empty-state">Прогноз временно недоступен.</div>';
      q('#salesHealth').innerHTML = '<div class="empty-state">Здоровье отдела временно недоступно.</div>';
      q('#funnelConversions').innerHTML = '<div class="empty-state">Конверсии временно недоступны.</div>';
    }
  }
