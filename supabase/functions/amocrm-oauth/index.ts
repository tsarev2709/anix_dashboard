...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error } = await supabase.from('integration_credentials').upsert({
      source_slug: 'amocrm', account_domain: domain, access_token: token.access_token,
      refresh_token: token.refresh_token, token_expires_at: expiresAt,
      metadata: { installed_at: new Date().toISOString() }, updated_at: new Date().toISOString(),
    });
    if (error) throw error;
    await supabase.from('data_sources').update({ status: 'warning', last_error: null, metadata: { account_domain: domain, oauth_connected: true } }).eq('slug', 'amocrm');

    return new Response('<html><body style="font-family:system-ui;padding:40px"><h1>amoCRM подключена</h1><p>Токены сохранены безопасно в Supabase. Можно закрыть окно и запустить синхронизацию.</p></body></html>', { headers: { ...cors, 'Content-Type': 'text/html; charset=utf-8' } });
  } catch (error) {
