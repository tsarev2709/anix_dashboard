...
const plans = [
  { key: 'touches', label: 'Целевые касания', plan: 1000, unit: 'шт.', source: 'tasks' },
  { key: 'conversations', label: 'Ответы / переписки', plan: 250, unit: 'шт.', source: 'missing', missing: 'Ответы клиентов пока нельзя надёжно отличить от исходящих сообщений. Нужна синхронизация коммуникаций amoCRM / почты / Telegram.' },
  { key: 'calls', label: 'Квалификационные звонки', plan: 25, unit: 'шт.', source: 'tasks' },
  { key: 'sql', label: 'Квалифицированные лиды / SQL', plan: 10, unit: 'шт.', source: 'stages', patterns: [/sql/i, /квалифиц/i, /вкс провед/i] },
  { key: 'proposals', label: 'Коммерческие предложения', plan: 5, unit: 'шт.', source: 'stages', patterns: [/\bкп\b/i, /коммерческ/i, /предложен/i] },
  { key: 'contracts', label: 'Договоры / счета', plan: 3, unit: 'шт.', source: 'stages', patterns: [/договор/i, /сч[её]т/i] },
  { key: 'prepayments', label: 'Предоплаты', plan: 2, unit: 'шт.', source: 'stages', patterns: [/предоплат/i, /аванс/i] },
  { key: 'payments', label: 'Оплаты', plan: 1000000, unit: '₽', source: 'stages', isMoney: true, patterns: [/оплат/i, /успешно реализован/i, /закрыт.*успеш/i] },
];
...
const text = (value: unknown) => String(value || '').toLocaleLowerCase('ru-RU');
  ...
  try {
    ...
    for (const lead of openLeads) {
      ...
      const current = byStatus.get(key) || {
        external_id: Number(lead.status_external_id || 0),
        name: status?.name || `Этап ${lead.status_external_id}`,
    ...
    const importedThisMonth = allLeads.filter((lead: any) => {
    ...
    for (const event of crmEvents || []) {
      if ((event.entity_type === 'lead' || event.entity_type === 'leads') && leadCreatedEventTypes.has(String(event.event_type || ''))) {
        operationalCreatedLeadIds.add(Number(event.entity_external_id));
      }
    }
    ...
    const firstTouchTasks = completedTasks.filter((task: any) => {
      const haystack = `${text(task.text)} ${text(task.result_text)}`;
      return /перв(ое|ый).*касани|перв.*сообщ|перв.*письм|холодн.*касани/.test(haystack);
    });
    const followUpTasks = completedTasks.filter((task: any) => /follow[- ]?up|фоллоу|повторн.*касани|напомн/.test(`${text(task.text)} ${text(task.result_text)}`));
    ...
    const kpis = plans.map((plan: any) => {
      if (plan.source === 'missing') return { ...plan, measurable: false, fact: null, forecast: null, completion: null, pace: null, status: 'missing' };

      if (plan.source === 'tasks') {
        const taskSet = plan.key === 'touches' ? [...firstTouchTasks, ...followUpTasks] : callTasks;
        const uniqueTaskIds = new Set(taskSet.map((task: any) => Number(task.external_id)));
        const fact = uniqueTaskIds.size;
        const forecast = Math.round(fact / elapsedRatio);
        const completion = fact / plan.plan;
        const forecastCompletion = forecast / plan.plan;
        return { ...plan, measurable: true, matched_statuses: [], task_breakdown: plan.key === 'touches' ? { first_touches: firstTouchTasks.length, follow_ups: followUpTasks.length } : { calls: callTasks.length }, fact, forecast, completion, forecast_completion: forecastCompletion, pace: plan.plan * elapsedRatio, status: forecastCompletion >= 1 ? 'green' : forecastCompletion >= .8 ? 'yellow' : forecastCompletion >= .6 ? 'orange' : 'red' };
      }

      const matchedStatuses = (statuses || []).filter((status: any) => plan.patterns?.some((pattern: RegExp) => pattern.test(status.name || '')));
      ...
      return { ...plan, measurable: true, matched_statuses: matchedStatuses.map((status: any) => ({ name: status.name, pipeline_name: pipelineMap.get(Number(status.pipeline_external_id))?.name || null })), fact, forecast, completion, forecast_completion: forecastCompletion, pace: plan.plan * elapsedRatio, status: forecastCompletion >= 1 ? 'green' : forecastCompletion >= .8 ? 'yellow' : forecastCompletion >= .6 ? 'orange' : 'red' };
    });

    const measurableKpis = kpis.filter((item: any) => item.measurable && item.forecast_completion !== null && (item.matched_statuses?.length || item.source === 'tasks'));
    ...
    for (const task of completedTasks) {
      const id = Number(task.responsible_user_external_id || 0);
      if (!id) continue;
      const user = userMap.get(id) as any;
      const row = responsible.get(id) || { id, name: managerName(id), email: user?.email || null, is_admin: Boolean(user?.is_admin), is_active: user?.is_active !== false, open: 0, created_month: 0, pipeline_value: 0, completed_tasks_month: 0 };
      row.completed_tasks_month += 1;
      responsible.set(id, row);
    }
    ...
    const upcoming = openLeads.map((lead: any) => {
      ...
      return nextAt ? { external_id: lead.external_id, name: lead.name || `Сделка #${lead.external_id}`, at: nextAt, status_name: findStatus(lead.pipeline_external_id, lead.status_external_id)?.name || null, pipeline_name: pipelineMap.get(Number(lead.pipeline_external_id))?.name || null, responsible_user_external_id: lead.responsible_user_external_id, responsible_user_name: managerName(lead.responsible_user_external_id) } : null;
    }).filter(Boolean).filter((item: any) => new Date(item.at) >= new Date(now.getTime() - 86400000)).sort((a: any, b: any) => new Date(a.at).getTime() - new Date(b.at).getTime()).slice(0, 10);
    ...
    const recent = [...allLeads].sort((a: any, b: any) => new Date(b.updated_at_source || 0).getTime() - new Date(a.updated_at_source || 0).getTime()).slice(0, 10)
      .map((lead: any) => ({ ...lead, raw: undefined, status_name: findStatus(lead.pipeline_external_id, lead.status_external_id)?.name || null, pipeline_name: pipelineMap.get(Number(lead.pipeline_external_id))?.name || null, responsible_user_name: managerName(lead.responsible_user_external_id) }));

    const missingData = [
      ...kpis.filter((item: any) => !item.measurable).map((item: any) => ({ metric: item.label, reason: item.missing, priority: 'high' })),
    ...
    return new Response(JSON.stringify({
      ...
      summary: {
        ...
        priced_open_leads: pricedOpenLeads.length,
        new_leads_month: operationalCreatedLeadIds.size,
        imported_or_created_month: importedThisMonth.length,
        overall_forecast: overallForecast,
        completed_tasks_month: completedTasks.length,
      },
      pipelines: pipelines || [],
      users: users || [],
      kpis,
