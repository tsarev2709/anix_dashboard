(() => {
  ...
  root.innerHTML = `<div class="forecast-heading"><div><p class="eyebrow">Продажи / месяц</p><h2>${compact ? 'Прогноз Anix' : 'Что закроем в этом месяце'}</h2><p>Будущие подписания договоров после проведённой встречи. Ранние стадии, предоплата и производство исключены. Факт — выигранные сделки; это не денежные поступления.</p></div>${compact ? '<a class="forecast-full" href="index.html#forecast" target="_blank" rel="noopener">Открыть полный Dashboard ↗</a>' : ''}</div>
  ...
  function render(payload) {
    ...
    if (d.signing_without_date) notices.push(`У ${d.signing_without_date} сделок после встречи нет даты подписания (сумма ${money(d.signing_without_date_amount)}). Они не распределены по месяцам и пока не входят в прогноз.`);
    if (d.unmapped_deals_count) notices.push(`Для ${d.unmapped_deals_count} открытых сделок ещё не сопоставлены стадии подписания. Они исключены из прогноза.`);
    ...
    result.innerHTML = notices.map(n => `<p class="forecast-warning" role="note">${esc(n)}</p>`).join('') +
      `<div class="forecast-kpis">${card('Факт месяца', money(a.amount), 'Выигранные сделки по дате закрытия')}${card('Взвешенный прогноз', money(f.weighted_forecast_amount), 'Будущие подписания × вероятность стадии')}${card('Потенциал сделок', money(f.potential_pipeline_amount), 'Подписания выбранного месяца')}</div>` +
    ...
    if (activity) result.insertAdjacentHTML('beforeend', panel('Действия продавца за месяц',
      `<p class="forecast-hint">Автор действия определяется по событию, а не по текущему владельцу сделки. ${esc(activity.coverage)}</p><div class="forecast-kpis forecast-secondary">${card('Зафиксировано действий', num(activity.total.actions))}${card('Выполнено задач', num(activity.total.completed_tasks), 'Отдельно; не прибавляем к касаниям')}${card('Ориентир на человека / день', num(activity.daily_target), 'Касания + поиск + анализ')}</div>` +
      table(['Дата', 'Сотрудник', 'Звонки', 'Письма', 'Сообщения / SMS', 'Поиск', 'Анализ', 'Действия / ориентир', 'Задачи'], activity.by_day.map(r => [day(r.date), esc(r.manager_name), num(r.calls), num(r.emails), num(r.messages+r.sms), num(r.contact_search), num(r.research), `${num(r.actions)} / ${num(r.target)}`, num(r.completed_tasks)]), 'Нет зафиксированных действий с известным автором за выбранный период.') +
      `<details><summary>Как учитывать поиск контактов и аналитику</summary><p class="forecast-hint">${esc(activity.recording)}</p><p class="forecast-hint">Завершение задачи «позвонить» не доказывает звонок. Звонки, письма и сообщения учитываются по событиям интеграций; автоматические изменения полей и импорт не учитываются. Пустые дни не считаются автоматически днями без работы. При фильтре воронки события без связи со сделкой исключаются.</p></details>`));
    if (!payload.configuration.signing_policy_applied_at) {
      result.insertAdjacentHTML('afterbegin', '<div class="forecast-warning"><p>Правила подписаний: встреча и подготовка ТКП — 50%; отправленное ТКП и согласование — 70%; договор — 80%. Дата обязательна в amoCRM после встречи. Будет добавлен «Формат контракта»: разовый, ежемесячный, рамочный.</p><button type="button" class="forecast-apply-policy">Подключить правила подписаний</button><p class="forecast-policy-status" role="status"></p></div>');
      const button = result.querySelector('.forecast-apply-policy');
      button.addEventListener('click', async () => {
        button.disabled = true;
        const message = result.querySelector('.forecast-policy-status');
        message.textContent = 'Настраиваю поля и стадии amoCRM…';
        try {
          const response = await fetch(endpoint, {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'apply_signing_policy'})});
          const body = await response.json();
          if (!response.ok || !body.ok) throw new Error(body.error || `HTTP ${response.status}`);
          await load();
        } catch (error) {message.textContent = `Не удалось применить: ${error.message}`;button.disabled=false;}
      });
    }
    const discipline = [['Без следующего шага', d.without_next_step], ['Просроченные задачи', d.overdue_tasks], ['Просрочена дата сделки', d.overdue_close_date], ['После встречи без даты', d.signing_without_date], ['Без активности >14 / >30 дней', `${d.stalled_14} / ${d.stalled_30}`]];
    ...
    result.insertAdjacentHTML('beforeend', panel('Качество данных', `<div class="forecast-kpis forecast-secondary">${card('Дата после встречи', pct(payload.quality.date_coverage))}${card('Есть следующий шаг', pct(payload.quality.next_step_coverage))}${card('Заполнен бюджет', pct(payload.quality.budget_coverage))}${card('Известна вероятность', pct(payload.quality.probability_coverage), 'Среди сделок выбранного месяца')}</div>`) +
      ...
      `<details class="panel forecast-settings"><summary>Вероятности и обязательность даты</summary><p class="forecast-hint">Проценты задаются для стадий, полученных из amoCRM. Пустое значение снимает ручную настройку. Обязательность даты в amoCRM настроена правилами подписаний. Флажок ниже управляет дополнительным контролем отчёта.</p><p class="forecast-hint">Поле даты: ${esc(field?.candidates?.map(c => c.name).join(', ') || 'не найдено')} · ${esc(field?.state || 'не синхронизировано')}. Историческая модель: ${payload.configuration.observed_enabled ? 'включена' : 'выключена'}, минимум ${num(payload.configuration.min_sample_size)} завершённых сделок.</p><div class="forecast-stage-settings">${payload.configuration.stages.filter(s => s.outcome === 'open').map(s => `<form class="forecast-stage-form" data-pipeline="${s.pipeline_external_id}" data-stage="${s.status_external_id}"><div><strong>${esc(s.name)}</strong><small>${esc(s.pipeline_name)} · ${esc(({signing:'В прогнозе подписаний',delivery:'Исполнение / оплата — исключено',excluded:'Ранняя стадия — исключено',unmapped:'Не сопоставлено — исключено'})[s.forecast_phase])} · по истории ${pct(s.observed_probability)} (${s.sample_size} сделок)</small></div><label>Вероятность, %<input name="probability" type="number" min="0" max="100" step="0.1" value="${s.manual_override == null ? '' : num(s.manual_override * 100).replace(',', '.')}" placeholder="Не задана"></label><label class="forecast-check"><input name="required" type="checkbox" ${s.requires_expected_date ? 'checked' : ''}> Нужна дата</label><button type="submit">Сохранить</button><span class="forecast-save-status" role="status"></span></form>`).join('')}</div></details>` +
